...
task_preload_game_libs() {
  ...
  while IFS= read -r lib; do
    ...
    if [ "$available_percent" -le "$RAM_SAFETY_THRESHOLD" ]; then
      post_notification "⚠️ Pre-caching" "Stopped early due to low RAM (${available_percent}%)"
      break
    fi
  ...
  wait
  post_notification "✅ Pre-caching" "Finished preloading $pkg"
}
...
task_restore_defaults() {
  post_notification "🌙 Normal Mode" "Returning to normal mode"
