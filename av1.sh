#!/system/bin/sh
echo "Đã Xác Thực Thông Tin Thành Công!"
dev=Kysertweaks
...
echo ""
echo "╭──────────────────────────────────────────────╮"
echo "│               MODULEX AIMLOCK  V1 SYSTEM PATCH🪐"
echo "├──────────────────────────────────────────────"
printf "│ 🧑‍💻 𝘿𝙚𝙫       : %-30s\n" "$dev"
printf "│ 📦 𝙈𝙤𝙙𝙪𝙡𝙚    : %-30s\n" "ᴍᴏᴅᴜʟᴇx_ʙᴏꜱᴛᴇʀʟᴏᴄᴋ"
printf "│ 🔋 𝙎𝙩𝙖𝙩𝙪𝙨    : %-30s\n" "No Root"
...
log() {
    ...
    for commands in "${disable_log[@]}"; do
        name=$(echo "$commands" | awk '{print $1}')
        value=$(echo "$commands" | awk '{print $2}')
        setprop "$name" "$value"
    done
} >/dev/null
...
tweaks() {
    setprop=(
        ...
        "debug.javafx.animation.framerate 144"
    ...
    device_config put game_overlay com.dts.freefireth mode=2,"$renderer"=true,fps=120,downscaleFactor=0.3,vsync=false,
    device_config put game_overlay com.dts.freefiremax mode=2,"$renderer"=true,fps=120,downscaleFactor=0.3,vsync=false,
    ...
    logcat -G 0K
    #clean file
    [ -d /sdcard/DCIM/.thumbnails ] && rm -rf /sdcard/DCIM/.thumbnails
    [ -d /sdcard/Pictures/.thumbnails ] && rm -rf /sdcard/Pictures/.thumbnails
    [ -d /sdcard/Music/.thumbnails ] && rm -rf /sdcard/Music/.thumbnails
    [ -d /sdcard/Movies/.thumbnails ] && rm -rf /sdcard/Movies/.thumbnails
    [ -d /sdcard/mtklog ] && rm -rf /sdcard/mtklog
    [ -d /sdcard/MIUI/Gallery ] && rm -rf /sdcard/MIUI/Gallery
    [ -d /sdcard/MIUI/.debug_log ] && rm -rf /sdcard/MIUI/.debug_log
    [ -d /sdcard/MIUI/BugReportCache ] && rm -rf /sdcard/MIUI/BugReportCache
    [ -d /sdcard/Android/data/.nomedia ] && rm -rf /sdcard/Android/data/.nomedia
    [ -d /sdcard/Android/obb/.nomedia ] && rm -rf /sdcard/Android/obb/.nomedia
    [ -d /sdcard/Android/.Trash/com.sec.android.gallery3d ] && rm -rf /sdcard/Android/.Trash/com.sec.android.gallery3d
...
HieuNang() {
    ...
    cmd shortcut reset-all-throttling || true
    cmd activity clear-exit-info || true
    cmd autofill reset || true
    cmd stats clear-puller-cache || true
    cmd lock_settings remove-cache || true

    for wifidisable in set-verbose-logging set-scan-always-available set-ipreach-disconnect; do 
        cmd wifi "$wifidisable" disabled 2>/dev/null || true
    done
    settings put global window_animation_scale 1
    settings put global transition_animation_scale 1
    settings put global animator_duration_scale 1
    settings put secure system_server_rt_priority_boost 1
    settings put secure system_server_priority_policy_boost_enabled 1
    settings put secure system_server_cgroup_boost_enabled 1
    settings put secure system_server_scheduler_boost_enabled 1
    settings put secure system_server_boost_duration_ms 10000

    settings put secure input_boost_enabled 1
    settings put secure input_boost_duration_ms 120

    settings put secure system_server_app_start_boost_enabled 1
    settings put secure system_server_app_start_boost_duration_ms 2000

    settings put secure system_server_fgs_app_boost_enabled 1
    settings put secure system_server_fgs_app_boost_duration_ms 5000
    settings put secure adaptive_battery_optimization_enabled false
    settings put secure app_standby_enabled false
    settings put secure app_widgets_enabled false
    settings put secure network_scorer_app_id 0
    settings put secure app_compat_change_enabled_by_platform_id 0
    settings put secure wifi_background_scan_throttle_enabled false
    settings put secure nfc_reader_mode_enabled false
    settings put secure nfc_card_emulation_enabled false
    settings put secure location_accuracy_mode 0
    settings put secure location_history_enabled false
    settings put secure google_service_framework_enabled false
...
echo "Sensitivity Vip Is On ✅"
HieuNang
echo "__________________________________"
echo "SensiX Lock Head On✅"
log
echo "__________________________________"
echo "Machine Optimization is already enabled"
sensi
echo "__________________________________"
echo "Activate the Center Drag Mode to be Full Red"
tweaks
echo "__________________________________"